...
public:
	...
	struct WFxParamPool {
		...
		void GetVector2(ID3DXEffect *ef, WFxParameterType wvdFxParamType, const char *hParam, bool isShared, D3DXVECTOR2 *Value) {
			if (!ef || !hParam) {
				return;
			}
			if (isShared) {
		...
		void GetVector3(ID3DXEffect *ef, WFxParameterType wvdFxParamType, const char *hParam, bool isShared, D3DXVECTOR3 *Value) {
			if (!ef || !hParam) {
				return;
			}
			if (isShared) {
		...
		void GetVector4(ID3DXEffect *ef, WFxParameterType wvdFxParamType, const char *hParam, bool isShared, D3DXVECTOR4 *Value) {
			if (!ef || !hParam) {
				return;
			}
			if (isShared) {
		...
		void GetMatrix(ID3DXEffect *ef, WFxParameterType wvdFxParamType, const char *hParam, bool isShared, D3DXMATRIX *Value) {
			if (!ef || !hParam) {
				return;
			}
			if (isShared) {
		...
		void GetTexture(ID3DXEffect *ef, WFxParameterType wvdFxParamType, const char *hParam, bool isShared, IDirect3DBaseTexture9 **Value) {
			if (!ef || !hParam) {
				return;
			}
			if (isShared) {
		...
		void SetVector2(ID3DXEffect *ef, WFxParameterType wvdFxParamType, const char *hParam, bool isShared, const D3DXVECTOR2 *Value) {
			if (isShared) {
				auto it = this->paramCacheListPairVec2.first.find(wvdFxParamType);
				if (it == this->paramCacheListPairVec2.first.end()) {
					this->paramCacheListPairVec2.first.insert(std::pair<WFxParameterType, D3DXVECTOR2>(wvdFxParamType, *Value));
				} else {
		...
		void SetVector3(ID3DXEffect *ef, WFxParameterType wvdFxParamType, const char *hParam, bool isShared, const D3DXVECTOR3 *Value) {
			if (isShared) {
				auto it = this->paramCacheListPairVec3.first.find(wvdFxParamType);
				if (it == this->paramCacheListPairVec3.first.end()) {
					this->paramCacheListPairVec3.first.insert(std::pair<WFxParameterType, D3DXVECTOR3>(wvdFxParamType, *Value));
				} else {
		...
		void SetVector4(ID3DXEffect *ef, WFxParameterType wvdFxParamType, const char *hParam, bool isShared, const D3DXVECTOR4 *Value) {
			if (isShared) {
				auto it = this->paramCacheListPairVec4.first.find(wvdFxParamType);
				if (it == this->paramCacheListPairVec4.first.end()) {
					this->paramCacheListPairVec4.first.insert(std::pair<WFxParameterType, D3DXVECTOR4>(wvdFxParamType, *Value));
				} else {
		...
		void SetMatrix(ID3DXEffect *ef, WFxParameterType wvdFxParamType, const char *hParam, bool isShared, const D3DXMATRIX *Value) {
			if (isShared) {
				auto it = this->paramCacheListPairMat.first.find(wvdFxParamType);
				if (it == this->paramCacheListPairMat.first.end()) {
					this->paramCacheListPairMat.first.insert(std::pair<WFxParameterType, D3DXMATRIX>(wvdFxParamType, *Value));
				} else {
		...
		void SetTexture(ID3DXEffect *ef, WFxParameterType wvdFxParamType, const char *hParam, bool isShared, IDirect3DBaseTexture9 *Value) {
			if (isShared) {
				auto it = this->paramCacheListPairTex.first.find(wvdFxParamType);
				if (it == this->paramCacheListPairTex.first.end()) {
					this->paramCacheListPairTex.first.insert(std::pair<WFxParameterType, IDirect3DBaseTexture9 *>(wvdFxParamType, Value));
				} else {
					if (it->second != Value) {
						it->second = Value;
					}
				...
				auto it = this->paramCacheListPairTex.second.find(hParam);
				if (it == this->paramCacheListPairTex.second.end()) {
					this->paramCacheListPairTex.second.insert(std::pair<const char *, IDirect3DBaseTexture9 *>(hParam, Value));
				} else {
					if (it->second != Value) {
						it->second = Value;
					}
	...
	private:
		template<typename T>
		inline static bool listSzEq(const T &a, const T &b) {
			return a.size() != b.size();
		}

		template<typename T>
		inline static bool rs2ListEq(const T &a, const T &b) {
...
NextKey:
		...
		template<typename T>
		inline static bool rs3ListEq(const T &a, const T &b) {
			for (class T::const_iterator i = a.begin(); i != a.end(); ++i) {
				for (class T::const_iterator j = b.begin(); j != b.end(); ++j) {
...
NextKey:
	...
	public:
		...
		bool operator==(const WRenderState &rhs) {
			...
			if (!listSzEq(this->m_rsList, rhs.m_rsList)) { return false; };
			if (!listSzEq(this->m_tssList, rhs.m_tssList)) { return false; };
			if (!listSzEq(this->m_transformList, rhs.m_transformList)) { return false; };
			if (!listSzEq(this->m_texList, rhs.m_texList)) { return false; };
			if (!listSzEq(this->m_clipPlaneList, rhs.m_clipPlaneList)) { return false; };
			if (!listSzEq(this->m_ssList, rhs.m_ssList)) { return false; };
			if (!listSzEq(this->m_fxParamIntList, rhs.m_fxParamIntList)) { return false; };
			if (!listSzEq(this->m_fxParamVec2List, rhs.m_fxParamVec2List)) { return false; };
			if (!listSzEq(this->m_fxParamVec3List, rhs.m_fxParamVec3List)) { return false; };
			if (!listSzEq(this->m_fxParamVec4List, rhs.m_fxParamVec4List)) { return false; };
			if (!listSzEq(this->m_fxParamMatList, rhs.m_fxParamMatList)) { return false; };
			if (!listSzEq(this->m_fxParamTexList, rhs.m_fxParamTexList)) { return false; };
			if (!rs2ListEq(this->m_rsList, rhs.m_rsList)) { return false; };
			if (!rs3ListEq(this->m_tssList, rhs.m_tssList)) { return false; };
			if (!rs2ListEq(this->m_transformList, rhs.m_transformList)) { return false; };
			if (!rs2ListEq(this->m_texList, rhs.m_texList)) { return false; };
			if (!rs2ListEq(this->m_clipPlaneList, rhs.m_clipPlaneList)) { return false; };
			if (!rs3ListEq(this->m_ssList, rhs.m_ssList)) { return false; };
			if (!rs2ListEq(this->m_fxParamIntList, rhs.m_fxParamIntList)) { return false; };
			if (!rs2ListEq(this->m_fxParamVec2List, rhs.m_fxParamVec2List)) { return false; };
			if (!rs2ListEq(this->m_fxParamVec3List, rhs.m_fxParamVec3List)) { return false; };
			if (!rs2ListEq(this->m_fxParamVec4List, rhs.m_fxParamVec4List)) { return false; };
			if (!rs2ListEq(this->m_fxParamMatList, rhs.m_fxParamMatList)) { return false; };
			if (!rs2ListEq(this->m_fxParamTexList, rhs.m_fxParamTexList)) { return false; };
		...
		bool HasAnyState() const {
			...
			if (!this->m_rsList.empty()) { return true; };
			if (!this->m_tssList.empty()) { return true; };
			if (!this->m_transformList.empty()) { return true; };
			if (!this->m_texList.empty()) { return true; };
			if (!this->m_ssList.empty()) { return true; };
			if (!this->m_clipPlaneList.empty()) { return true; };
			if (!this->m_fxParamIntList.empty()) { return true; };
			if (!this->m_fxParamVec2List.empty()) { return true; };
			if (!this->m_fxParamVec3List.empty()) { return true; };
			if (!this->m_fxParamVec4List.empty()) { return true; };
			if (!this->m_fxParamMatList.empty()) { return true; };
			if (!this->m_fxParamTexList.empty()) { return true; };
		...
		void Begin(WDirect3D8 *wd3d) {
			...
			for (auto it = this->m_clipPlaneList.begin(); it != this->m_clipPlaneList.end(); ++it) {
				...
				if ( it->m_SetValue.a != it->m_OldValue.a || it->m_SetValue.b != it->m_OldValue.b || it->m_SetValue.c != it->m_OldValue.c || it->m_SetValue.d != it->m_OldValue.d) {
			...
			for (auto it = this->m_fxParamIntList.begin(); it != this->m_fxParamIntList.end(); ++it) {
				...
				wd3d->m_fxParamPool.GetInt(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_OldValue);
				if (it->m_Value != it->m_OldValue) {
					wd3d->m_fxParamPool.SetInt(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, it->m_Value);
				}
			}
			...
			for (auto it = this->m_fxParamVec2List.begin(); it != this->m_fxParamVec2List.end(); ++it) {
				...
				wd3d->m_fxParamPool.GetVector2(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_OldValue);
				if (it->m_Value.x != it->m_OldValue.x || it->m_Value.y != it->m_OldValue.y) {
					wd3d->m_fxParamPool.SetVector2(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_Value);
				}
			}
			...
			for (auto it = this->m_fxParamVec3List.begin(); it != this->m_fxParamVec3List.end(); ++it) {
				...
				wd3d->m_fxParamPool.GetVector3(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_OldValue);
				if (it->m_Value.x != it->m_OldValue.x || it->m_Value.y != it->m_OldValue.y || it->m_Value.z != it->m_OldValue.z) {
					wd3d->m_fxParamPool.SetVector3(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_Value);
				}
			}
			...
			for (auto it = this->m_fxParamVec4List.begin(); it != this->m_fxParamVec4List.end(); ++it) {
				...
				wd3d->m_fxParamPool.GetVector4(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_OldValue);
				if (it->m_Value.x != it->m_OldValue.x || it->m_Value.y != it->m_OldValue.y || it->m_Value.z != it->m_OldValue.z || it->m_Value.w != it->m_OldValue.w) {
					wd3d->m_fxParamPool.SetVector4(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_Value);
				}
			}
			...
			for (auto it = this->m_fxParamMatList.begin(); it != this->m_fxParamMatList.end(); ++it) {
				...
				if (effect->pEffect && effect->param[it->m_Type].handle) {
					if (effect->param[it->m_Type].isShared) {
						...
						if (result != wd3d->m_fxParamPool.paramCacheListPairMat.first.end()) {
							...
							effect->pEffect->GetMatrix(static_cast<const char*>(effect->param[it->m_Type].handle), &it->m_OldValue);
						}
					} else {
						auto result = wd3d->m_fxParamPool.paramCacheListPairMat.second.find((char *)effect->param[it->m_Type].handle);
				...
				if (memcmp(&it->m_Value, &it->m_OldValue, 64)) {
					wd3d->m_fxParamPool.SetMatrix(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_Value);
				}
			}
			...
			for (auto it = this->m_fxParamTexList.begin(); it != this->m_fxParamTexList.end(); ++it) {
				...
				wd3d->m_fxParamPool.GetTexture(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_OldValue);
				if (it->m_Value != it->m_OldValue) {
					wd3d->m_fxParamPool.SetTexture(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, it->m_Value);
				}
				if (it->m_OldValue) {
					it->m_OldValue->Release();
				}
		...
		void End(WDirect3D8 *wd3d) {
			...
			for (auto it = this->m_fxParamIntList.begin(); it != this->m_fxParamIntList.end(); ++it) {
				...
				wd3d->m_fxParamPool.SetInt(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, it->m_OldValue);
			}
			...
			for (auto it = this->m_fxParamVec2List.begin(); it != this->m_fxParamVec2List.end(); ++it) {
				...
				wd3d->m_fxParamPool.SetVector2(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_OldValue);
			}
			...
			for (auto it = this->m_fxParamVec3List.begin(); it != this->m_fxParamVec3List.end(); ++it) {
				...
				wd3d->m_fxParamPool.SetVector3(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_OldValue);
			}
			...
			for (auto it = this->m_fxParamVec4List.begin(); it != this->m_fxParamVec4List.end(); ++it) {
				...
				wd3d->m_fxParamPool.SetVector4(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_OldValue);
			}
			...
			for (auto it = this->m_fxParamMatList.begin(); it != this->m_fxParamMatList.end(); ++it) {
				...
				wd3d->m_fxParamPool.SetMatrix(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, &it->m_OldValue);
			}
			...
			for (auto it = this->m_fxParamTexList.begin(); it != this->m_fxParamTexList.end(); ++it) {
				...
				wd3d->m_fxParamPool.SetTexture(effect->pEffect, it->m_Type, effect->param[it->m_Type].handle, effect->param[it->m_Type].isShared, it->m_OldValue);
			}
	...
	void DrawPrimitive(uint32_t iType, int iNum, uint32_t dwVertexTypeDesc, void *lpvVertices, D3DPRIMITIVETYPE dptPrimitiveType, int iType2) override;
	void DrawPrimitiveIndexed(int iType, uint32_t dwVertexTypeDesc, void *lpvVertices, int pNum, uint16_t* fList, int fNum, int iType2) override;
	...
	void CreateTextureSurface(uint32_t dwWidth, uint32_t dwHeight, int iMipmapCount, int renderTargetType, int iIndex, int bChromaKey, int bitNum) override;
...
protected:
	...
	void Release();
	void BackupRenderTarget(const WRenderToTextureParam &param, IDirect3DSurface9 *depthSurf, const D3DVIEWPORT9 &viewport);
	...
	void DrawIndexedPrimitiveUp(D3DPRIMITIVETYPE type, UINT numVertices, UINT primitiveCount, const void *pIndexData, const void *pVertexStreamZeroData, UINT vertexStreamZeroStride);
	void *SnapShotCustomRenderState() override;
	void ClearCustomRenderStateSnapShotList() override;
	void DrawPrimitiveUp(D3DPRIMITIVETYPE type, UINT primitiveCount, const void* pVertexStreamZeroData, UINT vertexStreamZeroStride);
	void DrawIndexedPrimitive(D3DPRIMITIVETYPE type, unsigned int minIndex, unsigned int numVertices, unsigned int startIndex, unsigned int primitiveCount);
	...
	void UpdateTextureSurfaceNormal(void *buff, int pitch, WDirect3D8::PixInfo *pix, BITMAPINFO *bi, void *data);
	void UpdateTextureSurfaceDirect(void *buff, int pitch, WDirect3D8::PixInfo *pix, BITMAPINFO *bi, void *data);
	void UpdateTextureSurfaceSampling(void *buff, int width, int height, int pitch, WDirect3D8::PixInfo *pix, BITMAPINFO *bi, void *data, int type);
	void UpdateTextureSurfaceFiltering(void *buff, int w, int h, int pitch, WDirect3D8::PixInfo *pix, BITMAPINFO *bi, uint8_t *data);
	void UpdateTextureSurfaceAlpha(void *buff, int pitch, WDirect3D8::PixInfo *pix, const tagRECT &rc, BITMAPINFO *bi, void *data);
	void UpdateTextureSurface(void *buff, int width, int height, int pitch, WDirect3D8::PixInfo *pix, BITMAPINFO *bi, void *data, int type);
...
private:
	...
	bool ApplyShader();
	void ReloadShader();
	bool DrawIndexedPrimitiveLockable(D3DPRIMITIVETYPE primitiveType, const void *vPtr, int vNum, int vPitch, uint16_t* iPtr, int primNum);
	bool DrawPrimitiveLockable(D3DPRIMITIVETYPE primitiveType, size_t primitiveCount, const void *pVertexStreamZeroData, unsigned int vertexStreamZeroStride);
	...
	static void __fastcall FillVertex(uint8_t*dest, const uint8_t*src, unsigned int xLastVertexDecl, size_t numVertices, unsigned int vertexStreamZeroStride, const D3DXMATRIX *m);
